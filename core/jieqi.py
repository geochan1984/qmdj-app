# -*- coding: utf-8 -*-
"""
jieqi.py - 節氣計算模組（純 Python 實現，使用 ephem 和 lunar-python）
已移除 sxtwl 依賴，改用 ephem 計算節氣，lunar-python 計算干支
"""
...
# ─── 節氣計算（用 ephem 替換 sxtwl）─────────────────────────────────────────

def _get_sun_longitude(date):
    """計算給定 ephem.Date 的太陽黃經（度）"""
    sun = ephem.Sun()
    sun.compute(date, epoch=date)
    ecl = ephem.Ecliptic(sun, epoch=date)
    return math.degrees(ecl.lon) % 360


def _get_solar_term_datetime_utc(year, target_lon):
    """用二分法精確計算節氣的 UTC 時間"""
    days_from_winter_solstice = ((target_lon - 270) % 360) / 360 * 365.25
    winter_solstice_approx = ephem.Date(f'{year - 1}/12/22')
    approx_date = winter_solstice_approx + days_from_winter_solstice
    start = approx_date - 15
    end = approx_date + 15
    mid = approx_date
    for _ in range(60):
        mid = (start + end) / 2
        ...
        if diff > 0:
            ...
            start = mid
    return ephem.Date(mid).datetime()


def _get_all_jieqi_for_year(year):
    """獲取某年所有節氣的日期時間（北京時間 UTC+8），按時間排序"""
    result = []
    for lon, name in zip(JIEQI_LONGITUDES, JIEQI_NAMES):
        dt_utc = _get_solar_term_datetime_utc(year, lon)
...
def get_jieqi_start_date(year, month, day, hour, minute):
    ...
    all_jieqi = (_get_all_jieqi_for_year(year - 1) +
                 _get_all_jieqi_for_year(year) +
                 _get_all_jieqi_for_year(year + 1))
...
# 農曆（用 lunar-python 替換 sxtwl.fromSolar）
